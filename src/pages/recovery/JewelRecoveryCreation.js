...
const handleImageDelete = (index, field) => {
 ...
 const handleChange = (e, fieldName, rowIndex) => {
  ...
  const formatInterestPaymentPeriods = (totalDays) => {
    ...
    if (days > 0) {
      periodString += `${months > 0 ? " " : ""}${days} day${days > 1 ? "s" : ""
        }`;
    }
  ...
  const handleSearchChange = (e, searchType) => {
    ...
    if (matchedPawnData) {
      ...
      setFormData((prevFormData) => ({
        ...
        refund_amount : parseFloat(matchedPawnData?.original_amount) + parseFloat(matchedPawnData.interest_payment_amount)
      }));
    } else {
      setFormData({
        ...formData,
      ...
      <Container>
        <Row className="regular">
          <Col lg="12" md="12" xs="12" className="py-3">
            <PageNav
              pagetitle={`Loan Closing${type === "view"
                  ? " view "
                  : type === "edit"
                    ? " edit "
                    : " Creation"
